...
This FastAPI app is designed to run on Render as a lightweight AI proxy that calls the Hugging Face Inference API and returns structured JSON drafts for the backend to consume.
...
2. Run: `uvicorn main:app --reload --host 0.0.0.0 --port 3000`

Deploy to Render
1. Create a new Web Service in Render, connect to this repo path `apps/backend/render-backend`.
2. Set Build Command: `pip install -r requirements.txt` and Start Command: `uvicorn main:app --host 0.0.0.0 --port $PORT`.
