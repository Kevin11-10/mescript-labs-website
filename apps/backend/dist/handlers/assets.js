...
export async function handleAssetDownload(request) {
    try {
        ...
        if (new Date() > new Date(record.expires_at)) {
            return new Response(JSON.stringify({ error: 'Download link expired' }), { status: 410, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        if (record.download_count >= record.max_downloads) {
            return new Response(JSON.stringify({ error: 'Maximum downloads reached' }), { status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        if (!response.ok) {
            return new Response(JSON.stringify({ error: 'Failed to fetch asset from GitHub' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
