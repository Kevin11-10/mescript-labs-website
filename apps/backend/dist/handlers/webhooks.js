...
async function verifyWebhookSignature(signature, body, secret) {
    try {
...
export async function handleCreemWebhook(request) {
    try {
        ...
        if (eventType === 'checkout.paid') {
            ...
            return new Response(JSON.stringify({ status: 'success', download_link: downloadLink }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        return new Response(JSON.stringify({ status: 'ignored_event' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    catch (error) {
        console.error('Webhook error:', error);
        return new Response(JSON.stringify({ error: 'Failed to process webhook' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
}
