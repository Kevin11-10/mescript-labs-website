...
export async function handleDirectUpload(request) {
    try {
        const env = request.env;
        const url = new URL(request.url);
        const key = url.searchParams.get('key');
        if (!key)
            return jsonResponse({ error: 'key query param required' }, 400);
        const hf = createHuggingFaceService(env);
...
export async function handleUploadStatus(request) {
    try {
        const url = new URL(request.url);
        const key = url.searchParams.get('key');
        if (!key)
