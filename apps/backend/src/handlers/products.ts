...
export async function handleProductList(request: Request): Promise<Response> {
  try {
...
export async function handleProductById(request: Request, productId: string): Promise<Response> {
  try {
    const env = (request as any).env as Env;
    const supabase = createSupabaseService({
      url: env.SUPABASE_URL,
